...
def parse_lines(lines_iter: Iterable[bytes]) -> Generator[str, None, None]:
    ...
    for line in lines_iter:
        ...
        if not line.startswith(b"data: "):
            raise UnknownError("wrong line: " + line)
...
def try_fetch_evil_api(state: EvilAPIState) -> EvilAPIState:
    ...
    source_url = (
        ...
        + "Free-ChatGPT-ChatBot/main/valid.json"
    )
    data = None
...
def try_fetch_evil_next_web(state: EvilNextWebState) -> EvilNextWebState:
    ...
    try:
        ...
        new_state["urls"] = [
            detail["url"]
            for detail in data
            if detail["type"] == "chatgpt_next_web"
        ]
        return new_state
    except (
        requests.RequestException,
...
def answer_stream_evil_next_web(
    ...
    headers = {"User-Agent": USER_AGENT}
    new_msg, new_state = append(msg, "assistant", ""), deepcopy(state)
    is_http_success = False
    errors = []
    for _ in range(5):
        url = choose_url(state["urls"], state["counts"])
        ...
        for delta in parse_lines(http_iter(resp.iter_lines())):
            new_msg[-1]["content"] += delta
            yield delta, new_msg, new_state
        return
    if is_http_success:
...
def answer_evil_next_web(
    ...
    for delta, ret_msg, ret_state in answer_stream_evil_next_web(
        msg, cfg, state
    ):
        answer += delta
...
def main():
    context = new_context_evil_next_web(
